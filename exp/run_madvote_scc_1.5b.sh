...
#   bash exp/run_madvote_scc_1.5b.sh
#   bash exp/run_madvote_scc_1.5b.sh --max_samples 5         # smoke test
#   ANALYZE_ONLY=1 ANALYZE_DIR=results_madvote_scc/run_<TS> \
#     bash exp/run_madvote_scc_1.5b.sh                       # rebuild tables
#   DATASETS="GSM8K AQUA-RAT" bash exp/run_madvote_scc_1.5b.sh
#   VARIANTS_FILTER="A0_vanilla A3_both" \
#     bash exp/run_madvote_scc_1.5b.sh                       # only run subset
# ==============================================================================
set -euo pipefail
...
OUTPUT_ROOT="${OUTPUT_ROOT:-./results_madvote_scc}"
ANALYZE_ONLY="${ANALYZE_ONLY:-0}"
ANALYZE_DIR="${ANALYZE_DIR:-}"
VARIANTS_FILTER="${VARIANTS_FILTER:-}"

# variant_id -> method_config_name mapping (yaml stem under
# methods/mad_vote_scc/configs/).
ALL_VARIANTS=(
...
# Pass-through args (--max_samples / --sequential / etc.) to inference.py.
PASSTHROUGH_ARGS=("$@")

# ------------------------------------------------------------------
# Resolve RUN_ROOT (top level for this 1.5B run; variants live underneath).
# ------------------------------------------------------------------
...
echo "=================================================="
echo "  Cluster A — 1.5B end-to-end pipeline"
echo "  Time      : $(date)"
...
echo ""
echo "=================================================="
echo ">> Cluster A 1.5B pipeline finished: $(date)"
echo ">> Run dir : ${RUN_ROOT}"
echo ">> Per-variant tables : ${RUN_ROOT}/<variant>/_tables/"
echo ">> Paper tables       : paper_tables/madvote_scc_A1_${MODEL_LABEL}.{csv,md}"
echo ">>                      paper_tables/madvote_scc_A2_${MODEL_LABEL}.{csv,md}"
echo "=================================================="
