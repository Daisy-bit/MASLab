#!/usr/bin/env bash
# ==============================================================================
# Cluster A — 7B end-to-end pipeline for `soo_scc` (selforg / DAG-cascade
# baseline + scc_components SCC modules). Mirrors run_soo_scc_1.5b.sh but
# targets the qwen25-7b-instruct model. Outputs land under results_soo_scc/.
#
#   Stage 1 (Inference)     : 6 variants x 5 datasets under
#                             results_soo_scc/run_<TS>/<variant>/
#       A0_vanilla       -> all four SCC flags off
...
#       A4_all           -> +Triggering +Routing +Aggregation (= v3 default)
...
#                             paper_tables/soo_scc_A1_7b.{csv,md}
#                             paper_tables/soo_scc_A2_7b.{csv,md}
#
# Round 0 is replayed from the SHARED initial pool at
#   results_archive/results_diagnostic/run_20260501_185633/qwen25-7b-instruct
# so all six variants debate from identical initial answer pools.
#
# NOTE: unlike the 1.5B pipeline (which uses a `_rejudged` pool), no rejudged
# pool exists yet for 7B. Override POOL_DIR if/when one becomes available.
#
# Diagnostic emit: enabled for all six variants. SOO-SCC inherits v3's
# `_inference_with_diagnostic` schema; downstream analyze_diagnostic.py
# consumes it the same way it consumes mad_vote / mad_scc JSONLs.
#
# Usage (from MASLab/ project root):
#   bash exp/run_soo_scc_7b.sh
#   bash exp/run_soo_scc_7b.sh --max_samples 5         # smoke test
#   ANALYZE_ONLY=1 ANALYZE_DIR=results_soo_scc/run_<TS> \
#     bash exp/run_soo_scc_7b.sh                       # rebuild tables
#   DATASETS="GSM8K AQUA-RAT" bash exp/run_soo_scc_7b.sh
#   VARIANTS_FILTER="A0_vanilla A4_all" bash exp/run_soo_scc_7b.sh
# ==============================================================================
set -euo pipefail
...
MODEL_LABEL="${MODEL_LABEL:-7b}"
METHOD_NAME="soo_scc"
OUTPUT_ROOT="${OUTPUT_ROOT:-./results_soo_scc}"
ANALYZE_ONLY="${ANALYZE_ONLY:-0}"
ANALYZE_DIR="${ANALYZE_DIR:-}"
VARIANTS_FILTER="${VARIANTS_FILTER:-}"
POOL_DIR="${POOL_DIR:-results_archive/results_diagnostic/run_20260501_185633/qwen25-7b-instruct}"
...
echo "=================================================="
echo "  Cluster A — 7B end-to-end pipeline (soo_scc)"
echo "  Time         : $(date)"
...
# soo_scc default `inference()` does NOT emit a diagnostic dict; we force
# emit_diagnostic=true via a per-call YAML override so downstream
# analyze_diagnostic can read its JSONL.
# ------------------------------------------------------------------
...
echo ""
echo "=================================================="
echo ">> Cluster A 7B pipeline (soo_scc) finished: $(date)"
echo ">> Run dir : ${RUN_ROOT}"
