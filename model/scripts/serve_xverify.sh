#!/bin/bash
# 使用 vLLM 部署 Qwen3.5-4B 作为 OpenAI 兼容 API 服务
...
#
# 用法:
#   bash serve_qwen.sh                    # 默认配置启动（使用 GPU 0）
#   bash serve_qwen.sh --gpu 1            # 指定使用 GPU 1
#   bash serve_qwen.sh --port 9000       # 指定端口
#   bash serve_qwen.sh --gpu-memory-utilization 0.9  # 调整 GPU 显存
#
# 也可通过环境变量指定显卡: VLLM_CUDA_DEVICES=1 bash serve_qwen.sh
#
# 启动后 API 地址: http://localhost:8081/v1
# 测试: python test_vllm_client.py

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
MODEL="${SCRIPT_DIR}/../xVerify-9B-C"
PORT="${VLLM_PORT:-8899}"
API_KEY="${VLLM_API_KEY:-token-abc123}"
# 指定使用的显卡，默认 GPU 0；可通过 --gpu 或环境变量 VLLM_CUDA_DEVICES 设置
CUDA_DEVICES="${VLLM_CUDA_DEVICES:-0}"
...
SERVED_MODEL_NAME="${VLLM_SERVED_MODEL_NAME:-xverify-9b-c}"
