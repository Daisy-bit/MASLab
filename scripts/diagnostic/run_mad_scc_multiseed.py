"""
Run mad_scc with a SHARED initial-response pool, repeated K seeds, then
aggregate results into mean ± std paper tables.

This is the v3-faithful sibling of run_madvote_scc_multiseed.py — it drives
the rebuilt `mad_scc` (methods.scc_components) instead of the legacy
`mad_vote_scc` and writes outputs under a separate filename pattern so
...
Layout produced (under --run_root):
  ...
  <run_root>/
    seed1/
      A0_vanilla/   mad_scc_<dataset>_infer.jsonl   (one per dataset)
        _tables/    table*.csv                       (analyze_diagnostic output)
      A1_triggering/...
      ...
      _paper/
        mad_scc_A1_<model_label>.csv                 (build_madvote_scc_tables output)
        mad_scc_A2_<model_label>.csv
    seed2/...
    seed3/...
    _aggregated/
      mad_scc_A1_<model_label>_mean.csv
      mad_scc_A1_<model_label>_meanstd.csv  (mean +/- std formatted)
      mad_scc_A2_<model_label>_mean.csv
      mad_scc_A2_<model_label>_meanstd.csv
...
Default --initial_pool_dir points to the archived diagnostic run from
...
Usage example (Windows PowerShell, from MASLab/ project root):
  ...
  python scripts/diagnostic/run_mad_scc_multiseed.py `
    --run_root results_mad_scc/run_<TS>_multiseed `
    --initial_pool_dir results_archive/results_diagnostic/run_20260501_113709/qwen25-1.5b-instruct_rejudged `
    ...
    --variants A0_vanilla A1_triggering A2_routing A3_aggregation A4_all A5_trig_rout
"""
...
def run_one_inference(
    *,
    ...
    out_path = os.path.join(out_dir, f"mad_scc_{dataset}_infer.jsonl")
...
def run_one_paper_tables(
    *,
    ...
    a1 = os.path.join(out_dir, f"mad_scc_A1_{label_lower}.csv")
    a2 = os.path.join(out_dir, f"mad_scc_A2_{label_lower}.csv")
    return a1, a2
...
def main() -> None:
    ...
    _aggregate_table(
        ...
        out_mean_path=os.path.join(
            agg_dir, f"mad_scc_A1_{label_lower}_mean.csv"
        ),
        out_meanstd_path=os.path.join(
            agg_dir, f"mad_scc_A1_{label_lower}_meanstd.csv"
        ),
    )
    _aggregate_table(
        seed_csv_paths=a2_paths,
        out_mean_path=os.path.join(
            agg_dir, f"mad_scc_A2_{label_lower}_mean.csv"
        ),
        out_meanstd_path=os.path.join(
            agg_dir, f"mad_scc_A2_{label_lower}_meanstd.csv"
        ),
    )
