...
class SelfOrg_Main(MAS):
    ...
    def __init__(self, general_config, method_config_name=None):
        ...
        self.num_agents = mc.get("num_agents", 5)
        ...
        # similarity pruning
        self.top_k = mc.get("top_k", 2)
        self.sim_threshold = mc.get("sim_threshold", -1)

        # DAG propagation
        self.max_rounds = mc.get("max_rounds", 2)
        self.enforce_dag = mc.get("enforce_dag", True)

        # aggregation
        self.aggregate_mode = mc.get("aggregate_mode", "weighted")
    ...
    def inference(self, sample):
        ...
        for i in range(self.num_agents):
            for j in helpful[i]:
                edges.add((j, i))
                adj = sims[i][j] * (1.0 + self.num_agents * (contributions[j] - contributions[i]))
    ...
    def _dagify(self, edges, edge_w):
        ...
        while True:
            adj = build_adj(E)
            color = {u: 0 for u in range(self.num_agents)}
